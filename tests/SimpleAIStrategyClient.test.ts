...
import SimpleAIStrategyClient from '../SimpleAIStrategyClient';
...
import playerAdded from '@civ-clone/civ1-treasury/Rules/Player/added';
...
import strategyPriority from '../Rules/Strategy/priority';
...
describe('SimpleAIStrategyClient', (): void => {
      ...
      Promise.all(
        new Array(n).fill(0).map(async (): Promise<Client> => {
          const player = new Player(ruleRegistry),
            client = new SimpleAIStrategyClient(player, strategyRegistry);
          ...
          const ChosenLeader = await client.chooseFromList(
            new ChoiceMeta(leaderRegistry.entries(), 'choose-leader')
          );
  ...
  ruleRegistry.register(
    ...
    ...playerAdded(playerTreasuryRegistry, cityBuildRegistry, ruleRegistry),
  ...
  strategyRegistry.register(
    new BuildCity(cityRegistry, ruleRegistry),
    new BuildDefender(ruleRegistry),
    new BuildExplorer(undefined, ruleRegistry),
    new BuildNavalTransportForAirUnits(ruleRegistry),
    new BuildNavalTransportForLandUnits(ruleRegistry),
    new BuildSettlers(cityGrowthRegistry, ruleRegistry, unitRegistry),
    new BuyDefender(playerTreasuryRegistry, ruleRegistry),
    ...
    new ChooseResearch(ruleRegistry, strategyRegistry, traitRegistry),
    new ChooseResearchFromList(undefined, ruleRegistry, traitRegistry),
    new ContinueOnPath(pathFinderRegistry, ruleRegistry, strategyNoteRegistry),
    new DoNothingWithUnit(ruleRegistry),
    new FortifyDefender(
      cityGrowthRegistry,
      cityRegistry,
      ruleRegistry,
    ...
    new ImproveTile(cityRegistry, ruleRegistry, tileImprovementRegistry),
  ...
  it('should build a city and defend it', async (): Promise<void> => {
    ...
    expect(unitRegistry.getByPlayer(player)).length(1);
    expect(unitRegistry.getByPlayer(player)[0]).eq(unit);

    await takeTurns(client);

    expect(unit.destroyed()).true;
    expect(unitRegistry.getByPlayer(player)).length(0);
    ...
    expect(cityBuild.building()!.item()).eq(Warrior);
    expect(cityBuild.progress().value()).eq(0);
